...
Start [mdns discovery](https://godoc.org/github.com/chiangmaioneluv/go-libp2p/p2p/discovery#NewMdnsService) service in host.

```go
ser, err := discovery.NewMdnsService(peerhost, rendezvous)
```
register [Notifee interface](https://godoc.org/github.com/chiangmaioneluv/go-libp2p/p2p/discovery#Notifee) with service so that we get notified about peer discovery
